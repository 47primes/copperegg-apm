  ...
  describe "#execute" do
    it "should benchmark time for a get request" do
      response = RestClient.get(
        "http://mikeapikey:U@api.rails.dev/v2/revealmetrics/metric_groups.json"
      )
      ...
      expect(hash["inst"]["url"]).to(
        eq("http://api.rails.dev/v2/revealmetrics/metric_groups.json {Ruby}")
      )
    ...
    it "should benchmark time for a post request" do
      response = RestClient.post(
        "http://mikeapikey:U@api.rails.dev/v2/revealmetrics/tags.json", tag: "asjasfe", ids: "smtp"
      )
    ...
    it "should benchmark time for a delete request" do
      response = RestClient.delete(
        "http://mikeapikey:U@api.rails.dev/v2/revealmetrics/tags/smtp.json", ids: "smtp"
      )
      ...
      expect(hash["inst"].keys.sort).to eq %w(time url)
      expect(hash["inst"]["url"]).to(
        eq("http://api.rails.dev/v2/revealmetrics/tags/smtp.json {Ruby}")
      )
      expect(hash["inst"]["time"].to_s).to match(/\A\d+\Z/)
    end
  end
