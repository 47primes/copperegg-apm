# Props to @relishapp
# http://www.relishapp.com/rspec/rspec-rails/docs/controller-specs/anonymous-controller)
# and
# @AlexandrZaytsev
# http://say26.com/rspec-testing-controllers-outside-of-a-rails-application)
# for their helpful blog posts
require "spec_helper"
...
describe ApplicationControllerSubclass, type: :controller do
  ...
  it "should instrument all exceptions not rescued" do
    ...
    expect(hash.keys.sort).to eq %w(excp id)
    expect(hash["id"]).to match(/\A[0-1a-z]{16}\z/i)
    expect(hash["excp"].keys.sort).to eq %w(error stacktrace ts)
    expect(hash["excp"]["error"]).to match(/ZeroDivisionError\|/)
    expect(hash["excp"]["stacktrace"]).to match(/\Adivided by 0\n/)
    expect(hash["excp"]["ts"]).to be_an_instance_of(Integer)
  end
end
