      ...
      module HTTP
        def request_with_ce_instrumentation(req, body = nil, &block)
          if CopperEgg::APM::Configuration.benchmark_http?
            ...
            url = "http#{'s' if @use_ssl}://#{address}\
              #{':#{port}' if port != ::Net::HTTP.default_port}#{req.path.sub(/\?.*/, '')}"
...
if defined?(::Net::HTTP)
  module Net
    class HTTP
      ...
      alias request_without_ce_instrumentation request
      alias request request_with_ce_instrumentation
    end
  end
end
