  ...
  module APM
    class BenchmarkMethodsTable
      ...
      def excluded_methods
        benchmarkable_methods.select(&:excluded?)
      end
      ...
      def separator
        "+" + "-" * (method_column_width + 1) + "+" +
          "-" * (source_filename_column_width + 1) + "+" + "--------------+"
      end

      def header_columns
        "| Method" + " " * (method_column_width - 6).abs + "| Source Location" +
          " " * (source_filename_column_width - 15).abs + "| Benchmarked? |"
      end
      ...
      def print_table
        if RUBY_VERSION < "1.9"
          puts "Method benchmarking is not available in Ruby #{RUBY_VERSION}. \
            It is only available in Ruby 1.9 or later."
        elsif CopperEgg::APM::Configuration.benchmark_methods_level == :disabled
          puts "Method benchmarking is disabled. You can enable \
            method benchmarking by setting the benchmark_methods configuration value."
        elsif benchmarkable_methods.empty?
          puts "No methods benchmarked"
        else
          puts
          puts "#{separator}\n#{header_columns}\n#{separator}\
            \n#{benchmarkable_methods.map { |m| method_columns(m) }.join('\n')}\n#{separator}"
          puts "#{benchmarkable_methods.size} methods defined in #{classes_count} \
            classes across #{file_count} files. #{benchmarkable_methods.size - excluded_methods.size == 1 ? '1 method' : "#{benchmarkable_methods.size - excluded_methods.size} methods"} benchmarked."
          puts
        end
      end
    end
